...
class DatabaseHelper {
  ...
  void _createDb(Database db, int newVersion) async {
    await db.execute('''Create TABLE tbl_person (
  ...
  Future<int> insertStudent(Student s) async {
    ...
    int result = await db.insert('tbl_person', s.toMap());
  ...
  Future<List<Student>> getAllStudents() async {
    ...
    Database db = await instance.database;
    //db.rawQuery('SELECT * from tbl_person');

    List<Map<String, dynamic>> listMap = await db.query('tbl_person');
  ...
  */
    int result = await db.update('tbl_person', s.toMap(), where: 'id=?', whereArgs: [s.id]);
    return result;
  }
  ...
  Future<int> deleteStudent(int id) async {
    ...
    int result = await db.delete('tbl_person', where: 'id=?', whereArgs: [id]);
    return result;
  }
}
